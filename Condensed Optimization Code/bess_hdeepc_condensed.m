%% BESS Simulation (Example 1, HDeePC)
% Jeremy Watson, 2025
%
...
for k = 1:lengthSim
    ...
    minimize quad_form(M(:,1) * q_meas + M(:,2:10) * Uf(1:m:N*m-m,:)*g-rq,Q2expanded) + quad_form(Yf*g-r,Qexpanded) + quad_form(Uf*g,Rexpanded) + lg * norm(g,1) + ly * norm(sigma_y,1) % Complete cost function

    [Up; Yp] * g == [uini; (yini + sigma_y)] % trajectories have to be consistent with initial data
    ...
    for i = 1:m:m*N
        ...
        Uf(i+1,:)*g == predictedGenLoad(1 + floor(i/2));
    end
    ...
    for i = 1:N
        ...
        M(i,:) * [q_meas; Uf(1:m:N*m-m,:)*g] >= 0
        M(i,:) * [q_meas; Uf(1:m:N*m-m,:)*g] <= 1
    end
