...
p_k = 2;                % Number of outputs for which the equation is known
p_u = p - p_k;          % Number of outputs for which the equation is unknown
n = size(A,1);          % Number of states
n_u = 1;                % Number of unknown states
...
L = place(A_k', C_k', desired_poles)';  % or use any appropriate method

% Initialize estimated state
x_hat = x(:, 1);%zeros(n,lengthSim+1);  % Could also initialize to zeros
for k = 1:lengthSim
