...
p_k = 1;                % Number of outputs for which the equation is known
p_u = p - p_k;          % Number of outputs for which the equation is unknown
n = size(A,1);          % Number of states
n_u = 2;                % Number of unknown states
...
A_y = A_c(1:n_k,1:p_u); % remember to adjust if y_u =/= x_u
C_y = C_c(1:p_k,1:p_u);
...
% Demonstrate the use of a Leuberger observer
% Observer gain (choose L such that A-L*C is stable)
% Desired observer poles (discrete-time, faster than system)
desired_poles = [ ...
    0.6+0.46i, 0.6-0.46i, ...
    0.65+0.35i, 0.65-0.35i, ...
    0.7+0.19i, 0.7-0.19i, ...
    0.5, 0.5 ...
];
L = place(A', C', desired_poles)';  % or use any appropriate method

% Initialize estimated state
x_hat = zeros(n,lengthSim+1);  % Could also initialize to zeros
for k = 1:lengthSim
    ...
    x_hat(:,k+1) = A * x_hat(:,k) + B * u_sim(:, k) + L * (y_sim(:, k) - C * x_hat(:,k) - D * u_sim(:, k));
